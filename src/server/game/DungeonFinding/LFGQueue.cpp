...
namespace lfg
{
    ...
    */
    ...
    void LFGQueue::AddToQueue(uint64 guid)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Queue data not found for [" UI64FMTD "]", guid);
            return;
        }
    ...
    */
    void LFGQueue::RemoveFromCompatibles(uint64 guid)
    {
        ...
        sLog->outDebug(LOG_FILTER_LFG, "Removing [" UI64FMTD "]", guid);
    ...
    */
    ...
    uint8 LFGQueue::FindGroups()
    {
        ...
        while (!newToQueueStore.empty())
        {
            ...
            sLog->outDebug(LOG_FILTER_LFG, "Checking [" UI64FMTD "] newToQueue(%u), currentQueue(%u)", frontguid, uint32(newToQueueStore.size()), uint32(currentQueueStore.size()));
            firstNew.clear();
    ...
    */
    LfgCompatibility LFGQueue::FindNewGroups(LfgGuidList& check, LfgGuidList& all)
    {
        ...
        sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s): %s - all(%s)", strGuids.c_str(), GetCompatibleString(compatibles), ConcatenateGuids(all).c_str());
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) compatibles (cached) changed from bad states to match", strGuids.c_str());
    ...
    */
    LfgCompatibility LFGQueue::CheckCompatibility(LfgGuidList check)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s): Size wrong - Not compatibles", strGuids.c_str());
        ...
        if (check.size() > 2)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) child %s not compatibles", strGuids.c_str(), ConcatenateGuids(check).c_str());
        ...
        for (LfgGuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 2 && numPlayers <= MAXGROUPSIZE; ++it)
        {
            ...
            if (itQueue == QueueDataStore.end())
            {
                sLog->outError(LOG_FILTER_LFG, "Guid: [" UI64FMTD "] is not queued but listed as queued!", guid);
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) sigle group. Compatibles", strGuids.c_str());
        ...
        if (numLfgGroups > 1)
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) More than one Lfggroup (%u)", strGuids.c_str(), numLfgGroups);
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) Too much players (%u)", strGuids.c_str(), numPlayers);
        ...
        if (check.size() > 1)
        {
                ...
                for (LfgRolesMap::const_iterator itRoles = roles.begin(); itRoles != roles.end(); ++itRoles)
                {
                    ...
                    for (itPlayer = proposalRoles.begin(); itPlayer != proposalRoles.end(); ++itPlayer)
                    {
                        if (itRoles->first == itPlayer->first)
                            sLog->outError(LOG_FILTER_LFG, "Guids: ERROR! Player multiple times in queue! [" UI64FMTD "]", itRoles->first);
            ...
            {
                sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) not compatible, %u players are ignoring each other", strGuids.c_str(), playersize);
            ...
            if (!LFGMgr::CheckGroupRoles(proposalRoles))
            {
                ...
                sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) Roles not compatible%s", strGuids.c_str(), o.str().c_str());
            ...
            {
                sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) No compatible dungeons%s", strGuids.c_str(), o.str().c_str());
        ...
        if (numPlayers != MAXGROUPSIZE)
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) Compatibles but not enough players(%u)", strGuids.c_str(), numPlayers);
            LfgCompatibilityData data(LFG_COMPATIBLES_WITH_LESS_PLAYERS);
        ...
        {
            sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) Group MATCH but can't create proposal!", strGuids.c_str());
        ...
        for (LfgRolesMap::const_iterator itRoles = proposalRoles.begin(); itRoles != proposalRoles.end(); ++itRoles)
        {
            ...
            data.groupLowGuid = proposalGroups.find(itRoles->first)->second;
            if (!proposal.isNew && data.groupLowGuid && data.groupLowGuid == proposal.groupLowGuid) // Player from existing group, autoaccept
        ...
        sLog->outDebug(LOG_FILTER_LFG, "Guids: (%s) MATCH! Group formed", strGuids.c_str());
        SetCompatibles(strGuids, LFG_COMPATIBLES_MATCH);
        return LFG_COMPATIBLES_MATCH;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_LFG, "Guid: " UI64FMTD, itrQueue->first);
        std::ostringstream o;
        o << itrQueue->first;
        std::string sguid = o.str();
    ...
    void LFGQueue::UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, std::string const& key, LfgRolesMap const& roles)
    {
        ...
        sLog->outDebug(LOG_FILTER_LFG, "Changed (%s) to (%s) as best compatible group for " UI64FMTD,
