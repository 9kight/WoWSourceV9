...
public:
	...
	private:
		...
		void MillhouseEvent_Face()
		{
			...
			for (std::vector<uint64>::const_iterator i = millhouseLastGroupGUIDs.begin(); i != millhouseLastGroupGUIDs.end(); ++i)
		...
		void MillhouseEvent_Knockback()
		{
			...
			for (std::vector<uint64>::const_iterator itr = millhouseLastGroupGUIDs.begin(); itr != millhouseLastGroupGUIDs.end(); ++itr)
		...
		void MillhouseEvent_Despawn()
		{
			...
			for (std::vector<uint64>::const_iterator itr = millhouseTrashGUIDs.begin(); itr != millhouseTrashGUIDs.end(); ++itr)
			if (Creature* creature = instance->GetCreature(*itr))
		...
		std::vector<uint64> millhouseTrashGUIDs;
		std::vector<uint64> millhouseLastGroupGUIDs;
		uint64 corborusRockDoorGUID;
		uint64 corobrusGUID;
		uint64 slabhideGUID;
		uint64 highPriestessAzilGUID;
		uint64 stonecoreTeleporterGUID[2];
		std::vector<uint64> slabhideRockWallGUIDs;
...
// 81235 - Ring Wyrm Knockback
class spell_ring_wyrm_knockback : public SpellScriptLoader
{
public:
	spell_ring_wyrm_knockback() : SpellScriptLoader("spell_ring_wyrm_knockback") { }

	class spell_ring_wyrm_knockback_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_ring_wyrm_knockback_SpellScript);

		void ModDest(SpellDestination& dest)
		{
			Position pos = { 1165.786f, 800.4249f, 285.0262f };
			dest.Relocate(pos);
		}
